...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "tenant_id", "repo_id", "qualified_name", name="unique_node_per_tenant"
        ),
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "tenant_id", "from_node", "to_node", "edge_type", name="unique_edge_per_tenant"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "embedding", postgresql.ARRAY(sa.Float(), dimensions=1), nullable=False
        ),  # Will be cast to vector(1024)
        sa.Column("metadata", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column("created_at", sa.TIMESTAMP(), server_default=sa.text("NOW()"), nullable=True),
        sa.Column("updated_at", sa.TIMESTAMP(), server_default=sa.text("NOW()"), nullable=True),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "tenant_id", "repo_id", "chunk_id", name="unique_embedding_per_tenant_repo"
        ),
    )

    # Cast embedding column to vector(1024) type
    op.execute(
        "ALTER TABLE embeddings ALTER COLUMN embedding TYPE vector(1024) USING embedding::vector(1024)"
    )
